...
class TransitionViewTests(TestCase):
    ...
    def test_new_view(self):
        ...
        data = {'name': 'Test Transition', 'from_state': self.state_two.id, 'to_state': self.state_one.id}
    ...
    def test_edit_view(self):
        transition = Transition.objects.create(
            workflow=self.workflow, from_state=self.state_one, to_state=self.state_two)
        response = self.client.get(reverse('forms:transition-edit',
                                   kwargs={"workflow_id": self.workflow.id, "transition_id": transition.id}))
        self.assertEqual(response.status_code, 200)

        new_data = {'name': 'Test Transition', 'from_state': self.state_two.id, 'to_state': self.state_one.id}
        response = self.client.post(reverse('forms:transition-edit',
                                    kwargs={"workflow_id": self.workflow.id, "transition_id": transition.id}), new_data)
        self.assertRedirects(response, reverse('forms:workflow-show', kwargs={"workflow_id": self.workflow.id}))
    ...
    def test_delete_view(self):
        ...
        response = self.client.get(reverse('forms:transition-delete',
                                   kwargs={"workflow_id": self.workflow.id, "transition_id": transition.id}))
        self.assertRedirects(response, reverse('forms:workflow-show', kwargs={"workflow_id": self.workflow.id}))
        self.assertEqual(Transition.objects.count(), transition_count_before - 1)
