...
class FormElementViewTests(TestCase):
    def setUp(self):
        ...
        self.form_element = FormElement.objects.create(
            workflow=self.workflow, caption="Name",
            hint="This is just a dummy field", position=1, element_type="text_input"
        )
    ...
    def test_edit_view(self):
        response = self.client.get(reverse('forms:form-element-edit',
                                           kwargs={
                                                "workflow_id": self.workflow.id,
                                                "element_id": self.form_element.id})
                                   )
        self.assertEqual(response.status_code, 200)

        new_data = {'caption': 'Occupation', 'hint': 'This is just a test', "position": 1, "element_type": "text_input"}
        response = self.client.post(reverse('forms:form-element-edit',
                                            kwargs={
                                                "workflow_id": self.workflow.id,
                                                "element_id": self.form_element.id}
                                            ),
    ...
    def test_delete_view(self):
        form_element_count_before = FormElement.objects.count()
        response = self.client.get(reverse('forms:form-element-delete',
                                           kwargs={
                                                "workflow_id": self.workflow.id,
                                                "element_id": self.form_element.id})
                                   )
