...
def workflow_entry_new(request, workflow_id):
    ...
    if request.method == 'POST':
        submission = WorkflowEntry(workflow_id=workflow_id, creator=request.user, current_state=workflow.initial_state())
        submission.save()
        for form_element in form_elements:
                submission.formelemententry_set.create(form_element=form_element, value=request.POST[form_element.caption])
