...
class UserViewTests(TestCase):
    ...
    def test_edit_view(self):
        response = self.client.get(reverse('home:user-edit', kwargs={"user_id": self.user.id}))
        self.assertEqual(response.status_code, 200)
        new_data = {'username': 'mary123', 'first_name': 'jane', "last_name": 'mary'}
        response = self.client.post(reverse('home:user-edit', kwargs={"user_id": self.user.id}), new_data)
