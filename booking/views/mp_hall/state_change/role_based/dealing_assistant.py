...
def generate_state_change_dealing_assistant(request, mp_hall_id):
    mp_hall = get_object_or_404(MpHall, id=mp_hall_id)
    remarks_error = ""
    if request.method == 'POST':
        if request.POST.get('APPROVED_BY_DA', False):
            if not can_proceed(mp_hall.approve_by_da):
            ...
            if not can_proceed(mp_hall.reject_by_da):
                raise PermissionDenied
            transition = TransitionHistory.objects.create(
                state_from=STATE.APPROVED_BY_HOD,
    ...
    return render(request, 'booking/mp_hall/state_change/role_based/dealing_assistant.html', context)
