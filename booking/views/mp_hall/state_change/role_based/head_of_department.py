...
def generate_state_change_head_of_department(request, mp_hall_id):
    mp_hall = get_object_or_404(MpHall, id=mp_hall_id)
    remarks_error = ""
    if request.method == 'POST':
        if request.POST.get('APPROVED_BY_HOD', False):
            if not can_proceed(mp_hall.approve_by_hod):
            ...
            if not can_proceed(mp_hall.reject_by_hod):
                raise PermissionDenied
            transition = TransitionHistory.objects.create(
                state_from=STATE.SUBMITTED,
    ...
    return render(request, 'booking/mp_hall/state_change/role_based/head_of_department.html', context)
