"""Test cases for testing authorization for Approval/Rejection by HOD."""
...
class PurchaseApprovalHODViewTests(TestCase):
    """Test cases for testing authorization for Approval/Rejection by HOD."""
    ...
    def test_purchase_indent_hod_approve_as_employee(self):
        """Employee who is not an HOD should not be able to approve form at HOD approval state."""
        ...
        response = self.client.post(reverse('purchase:purchase-indent-hod-approve',
                                    kwargs={"request_id": form.id}), {'Approve': 'Approve', 'remark': ''})
        self.assertEqual(response.status_code, 403)
        self.assertEqual(PurchaseIndentRequest.objects.filter(id=form.id)[0].state, 'Submitted')
    ...
    def test_purchase_indent_hod_approve_as_hod(self):
        """Employee who is HOD should be able to approve form at HOD approval state."""
    ...
    def test_purchase_indent_hod_reject_as_hod(self):
        """Employee who is HOD should be able to reject form at HOD approval state."""
        response = self.client.post(reverse('home:login'), {'username': 'hod_employee', 'password': 'qweasdzxc'})
        form = PurchaseIndentRequest.objects.create(indenter=self.employee, project_name="Project",
                                                    budget_head="Institute", state="Submitted")
    ...
    def test_purchase_indent_hod_approve_as_jao(self):
        """Employee(JAO) who is not an HOD should not be able to approve form at HOD approval state."""
    ...
    def test_purchase_indent_hod_approve_as_dr(self):
        """Employee(DR) who is not an HOD should not be able to approve form at HOD approval state."""
