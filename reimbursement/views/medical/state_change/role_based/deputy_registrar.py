...
def generate_state_change_deputy_registrar(request, medical_id):
    ...
    if request.method == 'POST':
        if request.POST.get('APPROVED_BY_DR', False):
            if not can_proceed(medical.approve_by_dr):
                raise PermissionDenied
            transition = MedicalTransitionHistory.objects.create(
                state_from=STATE.APPROVED_BY_MS,
            ...
            if not can_proceed(medical.reject_by_dr):
                raise PermissionDenied
            transition = MedicalTransitionHistory.objects.create(
                state_from=STATE.APPROVED_BY_MS,
    ...
    return render(request, 'reimbursement/medical/state_change/role_based/deputy_registrar.html', context)
