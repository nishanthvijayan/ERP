...
def generate_state_change_senior_audit_officer(request, telephone_expense_id):
    """
    Generates view for senior audit officer i.e SrAO for changing of states(Approve, Reject)
    This function assumes current user is authenticated as SrAO
    ...
    if request.method == 'POST':
        if request.POST.get('APPROVED_BY_SrAO', False):
            ...
            try:
                with transaction.atomic():
                    if not can_proceed(telephone_expense.reject_by_sr_ao):
                        raise PermissionDenied
                    transition = TelephoneExpenseTransitionHistory.objects.create(
                        state_from=STATE.APPROVED_BY_DR,
                    ...
                    transition.save()
                    telephone_expense.reject_by_sr_ao()
                    telephone_expense.save()
    ...
    return render(request, 'reimbursement/telephone_expense/state_change/role_based/senior_audit_officer.html', context)
