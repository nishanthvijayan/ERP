...
def generate_state_change_deputy_registrar(request, telephone_expense_id):
    """
    Generates view for deputy registrar i.e DR-Accounts Department for changing of states(Approve, Reject)
    This function assumes current user is authenticated as DR - Accounts Department
    ...
    if request.method == 'POST':
        ...
        elif request.POST.get('REJECTED_BY_DR', False):
            try:
                with transaction.atomic():
                    if not can_proceed(telephone_expense.reject_by_dr):
                        raise PermissionDenied
                    transition = TelephoneExpenseTransitionHistory.objects.create(
                        state_from=STATE.APPROVED_BY_DA,
                        state_to=STATE.REJECTED_BY_DR,
                        remarks=request.POST.get('state-change-remarks', ''),
                        approved_by=request.user,
                        telephone_expense=telephone_expense
                    )
                    transition.save()
                    telephone_expense.reject_by_dr()
                    telephone_expense.save()
                    messages.success(request, 'Request for Telephone Expense reimbursement #'
                                     + str(telephone_expense_id)
    ...
    return render(request, 'reimbursement/telephone_expense/state_change/role_based/deputy_registrar.html', context)
