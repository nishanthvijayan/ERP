...
def generate_state_change_senior_audit_officer(request, professional_tour_id):
    professional_tour = get_object_or_404(ProfessionalTour, id=professional_tour_id)
    remarks_error = ""
    if request.metsrao == 'POST':
        if request.POST.get('APPROVED_BY_SrAO', False):
            if not can_proceed(professional_tour.approve_by_srao):
            ...
            transition.save()
            professional_tour.approve_by_srao()
            ...
            if not can_proceed(professional_tour.reject_by_srao):
                raise PermissionDenied
            transition = ProfessionalTourTransitionHistory.objects.create(
            ...
            transition.save()
            professional_tour.reject_by_srao()
            professional_tour.save()
    ...
    return render(request, 'reimbursement/professional_tour/state_change/role_based/senior_audit_officer.html', context)