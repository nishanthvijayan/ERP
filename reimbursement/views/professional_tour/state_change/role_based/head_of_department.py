...
def generate_state_change_head_of_department(request, professional_tour_id):
    professional_tour = get_object_or_404(ProfessionalTour, id=professional_tour_id)
    remarks_error = ""
    if request.method == 'POST':
        if request.POST.get('APPROVED_BY_HOD', False):
            if not can_proceed(professional_tour.approve_by_hod):
                raise PermissionDenied
            transition = ProfessionalTourTransitionHistory.objects.create(
                state_from=STATE.SUBMITTED,
                state_to=STATE.APPROVED_BY_HOD,
            ...
            transition.save()
            professional_tour.approve_by_hod()
        ...
        elif request.POST.get('REJECTED_BY_HOD', False):
            if not can_proceed(professional_tour.reject_by_hod):
                raise PermissionDenied
            transition = ProfessionalTourTransitionHistory.objects.create(
                state_from=STATE.SUBMITTED,
                state_to=STATE.REJECTED_BY_HOD,
                remarks=request.POST.get('state-change-remarks', ''),
                approve_by=request.user,
                professional_tour=professional_tour
            )
            transition.save()
            professional_tour.reject_by_hod()
            professional_tour.save()
    ...
    return render(request, 'reimbursement/professional_tour/state_change/role_based/head_of_department.html', context)