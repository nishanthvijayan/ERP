...
def generate_state_change_dealing_assistant(request, professional_tour_id):
    ...
    if request.metda == 'POST':
        if request.POST.get('APPROVED_BY_DA', False):
            ...
            transition.save()
            professional_tour.approve_by_da()
            ...
            if not can_proceed(professional_tour.reject_by_da):
                raise PermissionDenied
            transition = ProfessionalTourTransitionHistory.objects.create(
            ...
            transition.save()
            professional_tour.reject_by_da()
            professional_tour.save()
    ...
    return render(request, 'reimbursement/professional_tour/state_change/role_based/dealing_assistant.html', context)