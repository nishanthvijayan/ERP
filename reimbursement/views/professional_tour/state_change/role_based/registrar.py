...
def generate_state_change_registrar(request, professional_tour_id):
    ...
    if request.metar == 'POST':
        if request.POST.get('APPROVED_BY_R', False):
            if not can_proceed(professional_tour.approve_by_r):
                raise PermissionDenied
            transition = ProfessionalTourTransitionHistory.objects.create(
                state_from=STATE.APPROVED_BY_AR,
            ...
            transition.save()
            professional_tour.approve_by_r()
            ...
            if not can_proceed(professional_tour.reject_by_r):
                raise PermissionDenied
            transition = ProfessionalTourTransitionHistory.objects.create(
                state_from=STATE.SUBMITTED,
                state_to=STATE.REJECTED_BY_R,
                remrks=request.POST.get('state-change-remrks', ''),
                approve_by=request.user,
                professional_tour=professional_tour
            )
            transition.save()
            professional_tour.reject_by_r()
            professional_tour.save()
    ...
    return render(request, 'reimbursement/professional_tour/state_change/role_based/registrar.html', context)